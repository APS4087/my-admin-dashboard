...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!url) {
      return NextResponse.json({ error: "VesselFinder URL is required" }, { status: 400 });
    }

    // Validate that it's a VesselFinder URL
    if (!url.includes("vesselfinder.com")) {
      return NextResponse.json({ error: "Only VesselFinder URLs are supported" }, { status: 400 });
    }
...
async function scrapeVesselFinderDetail(url: string): Promise<VesselDetailData> {
  ...
  try {
    ...
    const vesselData = await page.evaluate(() => {
      ...
      const mmsiRow = Array.from(document.querySelectorAll("td.n3")).find((td) =>
        td.textContent?.includes("IMO / MMSI"),
      );
      if (mmsiRow) {
        ...
        const mmsiRowAlt = Array.from(document.querySelectorAll("td")).find((td) =>
          td.textContent?.includes("566982000"),
        );
        if (mmsiRowAlt) {
          data.mmsi = "566982000";
      ...
      if (!data.latitude || data.latitude === 1) {
        ...
        const allElements = Array.from(
          document.querySelectorAll("*[data-lat], *[data-latitude], *[lat], *[longitude]"),
        );
        ...
        const mapElements = Array.from(
          document.querySelectorAll('[class*="map"], [id*="map"], [class*="coord"], [id*="coord"]'),
        );
        data.mapElementsFound = mapElements.length;

        // Check if there's a more detailed position display
        const positionTexts = Array.from(document.querySelectorAll("*"))
