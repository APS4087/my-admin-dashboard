...
export class ShipAuthService {
  ...
  async getShipAuthByEmail(email: string): Promise<ShipAuth | null> {
    const { data, error } = await this.supabase.from("ship_auth").select("*").eq("ship_email", email).single();
  ...
  async createShipAuth(authData: CreateShipAuthData): Promise<ShipAuth> {
    const { data, error } = await this.supabase.from("ship_auth").insert([authData]).select().single();
  ...
  async updateShipAuth(id: string, authData: UpdateShipAuthData): Promise<ShipAuth> {
    const { data, error } = await this.supabase.from("ship_auth").update(authData).eq("id", id).select().single();
  ...
  async deleteShipAuth(id: string): Promise<void> {
    const { error } = await this.supabase.from("ship_auth").delete().eq("id", id);
  ...
  async getStats() {
    const { data, error } = await this.supabase.from("ship_auth").select("is_active, ship_id");
