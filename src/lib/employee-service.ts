...
export class EmployeeService {
  ...
  async getAllEmployees(filters?: EmployeeFilters): Promise<Employee[]> {
    let query = this.supabase
      .from("employees")
      .select(
        "id, employee_number, email_address, first_name, last_name, display_name, department, office_phone, mobile_phone, job_title, is_active, created_at, updated_at, created_by, updated_by",
      )
    ...
    if (filters?.search) {
      query = query.or(
        `first_name.ilike.%${filters.search}%,last_name.ilike.%${filters.search}%,email_address.ilike.%${filters.search}%,job_title.ilike.%${filters.search}%`,
      );
    }
  ...
  async updateEmployee(id: string, employeeData: UpdateEmployeeData): Promise<Employee> {
    const { data, error } = await this.supabase.from("employees").update(employeeData).eq("id", id).select().single();
  ...
  async getEmployeeStats() {
    const { data, error } = await this.supabase.from("employees").select("is_active, department");
